...
对于输入向量 $\bold x$，输出 $\hat y = \text{sign}(\bold w \cdot \bold x)$。注意 $x_0 = 1$ 是我们额外加上的，这会使得 $w_0$ 在函数中起到**阈值**的作用，即
...
每次找到一个 $n(T)$ 使得 $\text{sign}(\bold w_T \cdot \bold x_{n(T)} + T) \neq y_{n(T)}$，令 $\bold w_{T + 1} \leftarrow \bold w_T + y_{n(T)}\bold x_{n(T)}$。此时第 ${n(T)}$ 组数据被修正。

当训练集线性可分（即存在一个 $w$ 使得所有训练集都被正确划分）时，PLA 会在一定时间后停止，否则会无限循环。

### 证明
设 $\bold w_f$ 能使得所有训练集都被正确划分，则这相当于 $p = \min_i \{y_i \bold w_f \cdot \bold x_i\} > 0$。不妨令 $\Vert \bold w_f \Vert = 1$。

再记 $R = \max_i \{\Vert \bold x_i \Vert\}$ 表示训练集半径。

令 $\bold w_0 = \bold 0$。

**引理 2.1** $\bold w_f \cdot \bold w_T > Tp$。

**证明 2.1** 通过调整，可以发现

$$
\begin{aligned}
  \bold w_f \cdot \bold w_{T + 1} &= \bold w_f \cdot (\bold w_T + y_{n(T)} \bold x_{n(T)}) \\
  &= \bold w_f \cdot \bold w_T + y_{n(T)} \bold w_f \bold x_{n(T)} \\
  &\geq \bold w_f \cdot \bold w_T + \min_i \{y_i \bold w_f \bold x_i\} \\
  &= \bold w_f \cdot \bold w_T + p \\
\end{aligned}
$$

故 $\bold w_f \cdot \bold w_T \geq \bold w_f \cdot \bold w_0 + Tp = Tp$。

**引理 2.2** $\Vert\bold w_T\Vert^2 \leq tR^2$。

**证明 2.2**

$$
\begin{aligned}
  \Vert \bold w_{T + 1} \Vert^2 &= \Vert \bold w_T + y_{n(T)}\bold x_{n(T)} \Vert^2 \\
  &= \Vert \bold w_T \Vert^2 + 2y_{n(T)}\bold w_T \cdot \bold x_{n(T)} + \Vert y_{n(T)} \bold x_{n(T)} \Vert^2 \\
  &\leq \Vert \bold w_T \Vert^2 + 0 + \Vert \bold x_{n(T)} \Vert^2 \\
  &\leq \Vert \bold w_T \Vert^2 + \max_i \Vert \bold x_i \Vert^2 \\
\end{aligned}
$$

故 $\Vert \bold w_T \Vert^2 \leq \Vert \bold w_0 \Vert^2 + TR^2 = TR^2$。

**定理 2.3** 令 $\bold w_0 = \bold 0$，则

$$\frac {\bold w_f \bold w_T}{\Vert \bold w_f\Vert \Vert \bold w_T \Vert} \geq \sqrt T \cdot \text{constant}$$

**证明 2.3** 

$$
\begin{aligned}
  \frac {\bold w_f \bold w_T}{\Vert \bold w_f\Vert \Vert \bold w_T \Vert} &\geq \frac {Tp}{\Vert \bold w_f \Vert \sqrt T R}  \\
  &\geq \sqrt T \cdot \frac pR  \\
\end{aligned}
$$

此外，注意到 $\frac {\bold w_f \bold w_T}{\Vert \bold w_f\Vert \Vert \bold w_T \Vert} = \cos \theta \leq 1$，因此也有
...
$$\bold w_f = \argmin_{\bold w} \sum_{n = 1}^N [y_n \neq \text{sign}(\bold w \cdot \bold x_n)]$$
...
也就是说，假设我们有 $N$ 个数据和一个确定的函数 $h: \bold x \rightarrow y$，则训练集误差 $E_{\text{in}}(h)$ 和测试集误差 $E_{\text{out}}(h)$ 会比较相近，满足
...
### Dichotomies
记 $\mathcal{H}(\bold x_1, \bold x_2, \ldots, \bold x_N) = (h(\bold x_1), h(\bold x_2), \ldots, h(\bold x_N)) \in \{1, -1\}^N$ 为固定 $\bold x_i$ 后将所有可能的 $h$ 对 $\bold x_i$ 取值得到的结果的集合，称之为一个 Dichtomy。

再定义**成长函数**（**Growth function**）$m_{\mathcal{H}}(N)$ 为在所有可能的 $\bold x_i$ 中，对不同 $h$ 产生结果种类的最大值

$$m_{\mathcal{H}}(N) = \max_{\bold x_1, \bold x_2, \ldots, \bold x_N} |\mathcal{H}(\bold x_1, \bold x_2, \ldots, \bold x_N)|$$
...
这样，我们考虑 $B(N, k)$ 时就不需要关心 $\mathcal H$ 是什么，只要找到最多的数据集 $\bold x_1, \bold x_2, \ldots, \bold x_M$ 使得仅凭任意 $k$ 个特征都不能分辨所有 $\bold x_i$。
...
定义配对：$\forall \bold a, \bold b \in S$，若 $\bold a, \bold b$ 在前 $N - 1$ 个特征都相等，则称 $\bold a, \bold b$ 有配对。
...
$$\text X = \begin{bmatrix}
  \bold x_1 \\ \bold x_2 \\ \vdots \\ \bold x_{k + 1}
\end{bmatrix}
...
要证明上述声明，相当于对于任意 $\bold y \in \{-1, 1\}^{k + 1}$，都能找到一个 $\bold w \in \R^{k + 1}$，使得

$$\text{sign}(\text X \bold w) = \bold y$$
...
注意到数据是 $k + 2$ 个 $k + 1$ 维向量，所以该向量组必定线性相关。不妨设 $\bold x_{k + 2} = \sum_{i = 1}^{k + 1} a_i \bold x_i$。

取输出 $\bold y$ 使得 $\forall 1 \leq i \leq k + 1, y_i = \text{sign}(a_i)$，则有

$$
\text{sign}(\bold w \cdot \bold x_i) = y_i = \text{sign}(a_i) \\
\Leftrightarrow \text{sign}(a_i\bold w \cdot \bold x_i) = 1 \\
 \Leftrightarrow a_i\bold w \cdot \bold x_i > 0
$$
...
$$
\begin{aligned}
  y_{k + 2} &= \text{sign}(\bold w \cdot \bold x_{k + 2}) \\
  &= \text{sign}\left(\sum_{i = 1}^{k + 1} a_i \bold w \cdot \bold x_i\right) \\
  &> \text{sign}\left(\sum_{i = 1}^{k + 1} 0 \right) \\
  &= 1 \\
\end{aligned}
$$

可见 $y_{k + 2}$ 在此情形下不可能为 $-1$，即我们无法让输出 $\bold y = \{\text{sign}(a_1), \text{sign}(a_2), \ldots, \text{sign}(a_{k + 1}), -1\}$。证毕。
